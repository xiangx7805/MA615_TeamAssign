...
pacman::p_load("leaflet","sp","magrittr","maps","htmltools","rgdal","ggplot2","maptools","XML","readr","rgeos","rmapshaper","tidyverse","lubridate","ggmap","ggrepel","dplyr",update = FALSE)
```
...
mayorsfoodcourt %>% filter(LICSTATUS == "Active") %>% dplyr::select(businessName,LICSTATUS,ViolLevel,VIOLDTTM,Address,CITY,ZIP,Location) %>% separate(Location, c("lat","lng"),",")-> foodcourt
...
foodcourt %>% dplyr::select(businessName,LICSTATUS,Address,CITY,ZIP,lat,lng) -> location
